...
//     * @param ignSinErr 是否忽略构建单个新增/编辑对象时的异常
//     * @return 如果 ignSinErr = true 的情况下，会把异常信息返回出来，否则则是一个空集合
//     */
//    List<Exception> editBatch(List<E> params, boolean ignSinErr);
//
//    /**
//     * 批量编辑[业务数据]表的数据【新增、编辑和删除】（对象的唯一标识只能是"id"）.
//     * @param params 待编辑的数据的集合
//     * @param query 根据入参集合构建查询对象，用于查询入参对应的全部旧数据
//     * @param mapKey Map的key生成器，主要用于查询结果构建Map，和入参对象从Map匹数据
//     * @param ignSinErr 是否忽略构建单个新增/编辑对象时的异常
//     * @return 如果 ignSinErr = true 的情况下，会把异常信息返回出来，否则则是一个空集合
//     */
//    List<Exception> editBatch1(List<E> params, Function<List<E>, Q> query, BiFunction<E, R, String> mapKey, boolean ignSinErr);
//
//    /**
//     * 基于指定的字段启用/禁用[业务数据].
